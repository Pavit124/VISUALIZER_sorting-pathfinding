...
function Visuals() {
   ...
   useEffect(() => {
      if(!myState.play){
         ...
         document.getElementById('play-btn').style.backgroundColor = '#554994;';
         document.getElementById('change-btn').disabled = false;
         document.getElementById('change-btn').style.backgroundColor = '#554994;';
      }
   },[myState.play]);
  ...
  return (
    <div className="visuals">
            ...
            <div className="visual__items" style={{width:`${myState.values.length*11}px`}}>
               {
                  myState.values.map((item) => {
                     ...
                     return <div className="visual__item" key={item[1]} id={item[1]} style={{transition:`${speed/1000}s linear all`, transform:`translateX(${item[1]*11}px)`}}>
                              <h4>{item[0]}</h4>
                              <div className="visual" style={{height:`${item[0]*5}px`, backgroundColor:color, width:(range<35? '8px': '6px')}}></div>
                           </div>
                  })
               }   
            </div>
      ...
      <div className="visual__btns">
         ...
         <button id = 'play-btn' onClick = {() => handlePlay(true)}>play</button>
